...
def get_list_path():   
    list_files = [item for item in os.listdir('./dataset') if '.xlsx' in item] # get list of filename
    list_path  = ['./dataset/'+item for item in list_files] # create full path   
    return list_path
...
def get_df_list(input_file_paths):
    ...
    for item in input_file_paths:
        dta = pd.read_excel(item, skiprows = 1) # skip one row due to empty first row
